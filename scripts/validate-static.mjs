...
const checks = [
  ['Vercel Preview serves the frontend and backend', vercelConfig.buildCommand === 'npm run vercel-build' && vercelConfig.outputDirectory === 'public' && Boolean(vercelConfig.functions?.['api/index.py']) && ['/api/:path*', '/health', '/docs/:path*', '/openapi.json'].every((source) => vercelRewrites.some((item) => item.source === source))],
