...
- Всегда различай: достоверное числовое 0, неизвестную/некорректную метрику и малую выборку.
- Если Директ не вернул данные по выбранным целям как числовую метрику, не называй это нулём конверсий: анализ CPA ограничен, нужны проверка ID целей и повторная синхронизация.
- При неизвестной конверсионной метрике продолжай независимый анализ качества трафика по доступным запросам, площадкам, аудиториям, устройствам, географии, объявлениям и частоте, но не связывай эти сигналы с продажами без evidence.
- Используй campaignDecisionScenarios как обязательные backend-сценарии: соблюдай forbiddenClaims и не подменяй отсутствующую метрику нулём.
