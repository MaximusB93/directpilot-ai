...
class DirectReportJob(Base):
    ...
    queue_full_attempts: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    first_queue_full_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    last_queue_full_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    rows_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
