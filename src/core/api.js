...
function isCacheableGet(path, options = {}) {
  ...
  return API_CACHEABLE_PATHS.some((prefix) => normalizedPath === prefix || normalizedPath.startsWith(`${prefix}/`) || normalizedPath.startsWith(`${prefix}?`));
}
...
function makeJsonResponse(payload, { cached = false, stale = false } = {}) {
  return new Response(JSON.stringify(payload), {
    status: 200,
    statusText: cached ? 'OK (DirectPilot cache)' : 'OK',
    headers: {
      'Content-Type': 'application/json',
      'X-DirectPilot-Cache': cached ? (stale ? 'stale' : 'fresh') : 'miss',
    },
  });
}
...
async function writeApiCache(path, response) {
  try {
    ...
    // Cache is an optimization only. Users have suffered enough already.
  }
}
...
function refreshApiCacheInBackground(path, options) {
...
export async function apiFetch(path, options = {}) {
  ...
  if (cacheable) {
    ...
    if (cached?.fresh) {
      return makeJsonResponse(cached.payload, { cached: true });
    }
    if (cached) {
      refreshApiCacheInBackground(path, options);
      return makeJsonResponse(cached.payload, { cached: true, stale: true });
    }
  }
