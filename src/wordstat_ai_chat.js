...
function getSelectedAiSettings() {
  try {
    ...
    return { model: model || null, ai_preset: saved.selectedPreset ? String(saved.selectedPreset) : null, max_tokens: 2500 };
  } catch {
...
function collectWordstatContext() {
  const payload = readStoredWordstatPayload();
...
function currentModelLabel() {
  return getSelectedAiSettings().model || 'модель из backend по умолчанию';
}
